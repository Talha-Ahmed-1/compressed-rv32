...
#endif
#include <iostream>
class CompressedDecoder_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    CompressedDecoder_api_t(VCompressedDecoder* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_instIn)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_compressed)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_instOut)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["CompressedDecoder.reset"] = 0;
    }
#if VM_TRACE
...
static CompressedDecoder_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VCompressedDecoder* top = new VCompressedDecoder;
    std::string vcdfile = "test_run_dir/Compressed_Decoder_Test/CompressedDecoder.vcd";
...
#endif
    CompressedDecoder_api_t api(top);
    ...
    Verilated::mkdir("test_run_dir/Compressed_Decoder_Test/logs");
    VerilatedCov::write("test_run_dir/Compressed_Decoder_Test/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
